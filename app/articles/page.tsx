...
export const metadata = {
  title: "Articles for Homeowners | Ultra Homebuyers",
  description:
    ...
    <div className="bg-white text-[#1a1a1a]">
      <main className="mx-auto max-w-[760px] px-6 pt-12 pb-24">
        ...
        <div className="grid grid-cols-1 gap-5">
          {ARTICLES.map((a) => (
            <Link
              key={a.slug}
              href={a.slug}
              className="group flex flex-col sm:flex-row overflow-hidden rounded-[14px] border border-[#e5e5e5] transition-colors hover:border-[#1a3d6b] hover:bg-[#f4f7fb] no-underline"
            >
              <Image
                src={a.image}
                alt={a.title}
              ...
              <div className="p-6">
                ...
                <span className="text-[15px] font-semibold text-[#1a3d6b]">Read the article →</span>
              </div>
            </Link>
          ))}
        </div>
        ...
        <footer className="mt-12 pt-6 border-t border-[#e5e5e5] text-[13px] text-[#6b6b6b] text-center">
          ...
          <p>
            <Link href={config.privacyPolicyUrl} className="text-[#1a3d6b] underline">
              Privacy
            </Link>
            {" · "}
            <Link href={config.termsUrl} className="text-[#1a3d6b] underline">
              Terms
            </Link>
          </p>
        </footer>
