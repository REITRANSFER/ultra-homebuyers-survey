...
// reader always has somewhere to go.
    ...
    <section className="border-t border-[#e5e5e5] bg-[#fafafa]">
      <div className="mx-auto max-w-[760px] px-6 py-12">
        ...
        <div className="grid grid-cols-1 gap-4">
          {next.map((a) => (
            <Link
              key={a.slug}
              href={a.slug}
              className="group flex flex-col sm:flex-row overflow-hidden rounded-[14px] border border-[#e5e5e5] bg-white transition-colors hover:border-[#1a3d6b] hover:bg-[#f4f7fb] no-underline"
            >
              <Image
                src={a.image}
                alt={a.title}
              ...
              <div className="p-5">
                ...
                <span className="text-[14px] font-semibold text-[#1a3d6b]">Read the article →</span>
              </div>
            </Link>
          ))}
        </div>
        ...
        <div className="mt-6 text-center">
          <Link href="/articles" className="text-[15px] font-semibold text-[#1a3d6b] underline">
            See all articles
          </Link>
        </div>
      </div>
